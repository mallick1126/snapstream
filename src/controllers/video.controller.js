...
import { uploadVideoObjectToS3, uploadObjectToS3 } from "../utils/s3.config.js";
import fs from "fs";
...
const publishAVideoS3 = asyncHandler(async (req, res) => {
  const userId = req.user?._id;
  const { title, description } = req.body;
  if (!title || !description) {
    throw new ApiError(400, "Title and description are required");
  ...
  const videoStream = fs.createReadStream(videoLocalPath);
  const thumbnailBuffer = fs.readFileSync(thumbnailLocalPath);

  const actualDuration = formatDuration(req.files.videoFile[0].duration);

  const videoFileS3Key = `videos/${Date.now()}-${
    req.files.videoFile[0].filename
  }`;
  const videoFileUrl = await uploadVideoObjectToS3(
    videoFileS3Key,
    videoStream,
    req.files.videoFile[0].mimetype
  );

  const thumbnailS3Key = `thumbnails/${Date.now()}-${
    req.files.thumbnail[0].filename
  }`;
  const thumbnailFile = await uploadObjectToS3(
    thumbnailS3Key,
    thumbnailBuffer,
  ...
  const newVideo = await Video.create({
    title,
    description,
    videoFile: videoFileUrl.Location,
